...
Meteor.methods({
  createActivity: function(activity, tweet, loc) {
    ...
    activity.userAvatar = Meteor.user().services && Meteor.user().services.twitter && Meteor.user().services.twitter.profile_image_url_https || '/img/user/avatar.png';
    //activity.userName = Meteor.user().profile.name;
    //activity.userName = Meteor.user().profile && Metor.user().profile.name
    activity.userName = Meteor.user().profile && Meteor.user().profile.name || 'Guest'
